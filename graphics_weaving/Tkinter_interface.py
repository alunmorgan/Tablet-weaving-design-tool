...
class Interface:
    ...
    def update_draft_grid(self):
        ...
        for x in range(self.num_cards):
            for y in range(self.draft_height):
                self.draft_grid_state[x][y].configure(bg=new_draft[x][y])
    ...
    def update_turning_grid(self):
        for x in range(self.length_turning_instructions):
            for y in range(self.num_cards):
                self.turning_grid_state[x][y].configure(bg=convert_numeric_to_colours(
                    convert_turns_to_numeric(self.input_data['Turning_instructions'][x][y])))
    ...
    def make_output_grid(self, x_offset, y_offset, tb_select):
        ...
        for x in range(2 * self.length_turning_instructions):
            ...
            for y in range(self.num_cards):
                ...
                y2 = y1 + cellheight
                if tb_select.casefold() == 'top'.casefold():
                    canvas.create_rectangle(x1, y1, x2, y2, fill=self.output_data['top_cols'][x][y], tags="rect")
                elif tb_select.casefold() == 'bottom'.casefold():
                    canvas.create_rectangle(x1, y1, x2, y2, fill=self.output_data['bottom_cols'][x][y], tags="rect")
                else:
